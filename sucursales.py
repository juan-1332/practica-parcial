...
def crear_sucursal(lista_sucursales):
    ...
    # ID autoincrementable basado en el máximo ID actual para evitar duplicados
    nuevo_id = max([s["id"] for s in lista_sucursales], default=0) + 1
...
def eliminar_sucursal(lista_sucursales):
    """Elimina una sucursal seleccionada por su ID previa confirmación."""
    if not lista_sucursales:
        print("\n No hay sucursales registradas para eliminar.")
        return

    try:
        id_buscar = int(input("\nIngrese el ID de la sucursal que desea eliminar: ").strip())
    except ValueError:
        print(" Error: El ID debe ser un número entero.")
        return
    ...
    confirmacion = input(f"¿Está seguro de eliminar la sucursal '{sucursal['nombre']}'? (s/n): ").strip().lower()
    if confirmacion == "s":
        lista_sucursales.remove(sucursal)
        print(f" Sucursal ID {id_buscar} eliminada con éxito.")
    else:
